...
{
    public class PiecearchivesController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(PiecearchiveWrap cararchiveWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Piecearchive model = cararchiveWrap.Piecearchive;
            ...
            else
            {
                ...
                var msg = new { Piecearchive = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
        ...
        private bool tryValidate(Model.Piecearchive model, out string errMsg)
        {
            errMsg = "";
            bool re = true;
            if (model.Piece == 0
        ...
        private bool tryPost(ref Model.Piecearchive model, out string errMsg)
        {
            errMsg = "";
            bool re = false;
            model.Creater = this.auth.UserId;
            model.CreatedDate = DateTime.Now;
            if (PieceManager.ArchivePiece(ref model, out errMsg))
            {
                re = true;
            }
            return re;
