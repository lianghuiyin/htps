...
{
    public class TracerecapturesController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(TracerecaptureWrap tracerecaptureWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Tracerecapture model = tracerecaptureWrap.Tracerecapture;
            ...
            else
            {
                ...
                var msg = new { Tracerecapture = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
        ...
        private bool tryValidate(Model.Tracerecapture model, out string errMsg)
        {
            errMsg = "";
            bool re = true;
            if (model.Trace == 0 || model.Instance == 0 || model.Car == 0
                || model.EndInfo.Length > 200
                || model.Creater == 0)
            {
                errMsg = "输入数据不合法";
                re = false;
        ...
        private bool tryPost(ref Model.Tracerecapture model, out string errMsg)
        {
            errMsg = "";
            bool re = false;
            model.Creater = this.auth.UserId;
            model.CreatedDate = DateTime.Now;
            if (InstanceManager.RecaptureTrace(ref model, out errMsg))
            {
                re = true;
            }
            return re;
