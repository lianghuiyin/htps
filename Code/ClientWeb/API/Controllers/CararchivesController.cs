...
{
    public class CararchivesController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(CararchiveWrap cararchiveWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Cararchive model = cararchiveWrap.Cararchive;
            ...
            else
            {
                ...
                var msg = new { Cararchive = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
        ...
        private bool tryValidate(Model.Cararchive model, out string errMsg)
        {
            errMsg = "";
            bool re = true;
            if (model.Car == 0
        ...
        private bool tryPost(ref Model.Cararchive model, out string errMsg)
        {
            errMsg = "";
            bool re = false;
            model.Creater = this.auth.UserId;
            model.CreatedDate = DateTime.Now;
            if (CarManager.ArchiveCar(ref model, out errMsg))
            {
                re = true;
            }
            return re;
