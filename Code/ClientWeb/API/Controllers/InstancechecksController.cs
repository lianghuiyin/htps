...
{
    public class InstancechecksController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(InstancecheckWrap instancecheckWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Instancecheck model = instancecheckWrap.Instancecheck;
            ...
            else
            {
                ...
                var msg = new { Instancecheck = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
        ...
        private bool tryValidate(Model.Instancecheck model, out string errMsg)
        {
            errMsg = "";
            bool re = true;
            if (model.Trace == 0
        ...
        private bool tryPost(ref Model.Instancecheck model, out string errMsg)
        {
            errMsg = "";
            bool re = false;
            model.Creater = this.auth.UserId;
            model.CreatedDate = DateTime.Now;
            if (InstanceManager.CheckInstance(ref model, out errMsg))
            {
                re = true;
            }
            return re;
