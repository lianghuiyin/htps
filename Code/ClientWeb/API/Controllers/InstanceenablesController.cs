...
{
    public class InstanceenablesController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(InstanceenableWrap instanceenableWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Instanceenable model = instanceenableWrap.Instanceenable;
            ...
            else
            {
                ...
                var msg = new { Instanceenable = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
        ...
        private bool tryValidate(Model.Instanceenable model, out string errMsg)
        {
            errMsg = "";
            bool re = true;
            if (model.Trace == 0
        ...
        private bool tryPost(ref Model.Instanceenable model, out string errMsg)
        {
            errMsg = "";
            bool re = false;
            model.Creater = this.auth.UserId;
            model.CreatedDate = DateTime.Now;
            if (InstanceManager.EnableInstance(ref model, out errMsg))
            {
                re = true;
            }
            return re;
