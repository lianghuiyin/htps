...
{
    public class TracenewsController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(TracenewWrap tracenewWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Tracenew model = tracenewWrap.Tracenew;
            ...
            else
            {
                ...
                var msg = new { Tracenew = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
