...
{
    public class PiecerestoresController : ApiController
    {
        private Authorizer auth;

        public HttpResponseMessage Post(PiecerestoreWrap piecerestoreWrap)
        {
            string errMsg = "";
            string errMsgForChangeset = "";
            bool isChangesetError = false;
            Piecerestore model = piecerestoreWrap.Piecerestore;
            ...
            else
            {
                ...
                var msg = new { Piecerestore = model, Changeset = changeset, IsChangesetError = isChangesetError, ErrMsgForChangeset = errMsgForChangeset };
                json = JsonConvert.SerializeObject(msg);
            }
        ...
        private bool tryValidate(Model.Piecerestore model, out string errMsg)
        {
            errMsg = "";
            bool re = true;
            if (model.Piece == 0
